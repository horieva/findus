...
class Findus_Apus_Userinfo{
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_findus_process_change_profile_form', array($this, 'process_change_profile_form') );
		add_action( 'wp_ajax_nopriv_findus_process_change_profile_form',  array($this, 'process_change_profile_form') );

		add_action( 'wp_ajax_findus_process_change_password', array($this, 'process_change_password') );
		add_action( 'wp_ajax_nopriv_findus_process_change_password',  array($this, 'process_change_password') );
		...
		add_filter( 'findus_new_user_approve_validate_status_update', array( __CLASS__, 'validate_status_update' ), 10, 3 );

		add_action( 'findus_new_user_approve_approve_user', array( __CLASS__, 'approve_user' ) );
		add_action( 'findus_new_user_approve_deny_user', array( __CLASS__, 'deny_user' ) );

		// resend approve account
		add_action( 'wp_ajax_findus_ajax_resend_approve_account',  array(__CLASS__,'process_resend_approve_account') );
		add_action( 'wp_ajax_nopriv_findus_ajax_resend_approve_account',  array(__CLASS__,'process_resend_approve_account') );
		...
		add_shortcode( 'findus_approve_user', array( __CLASS__, 'approve_user_shortcode' ) );
	}
	...
	public function processLogin() {
        ...
        if ( $user_login_auth == 'pending' && isset($user_obj->ID) ) {
        	...
        	echo json_encode(array(
            	'loggedin' => false,
            	'msg' => esc_html__('Your account denied', 'findus')
            ));
            die();
        }
	    ...
	    if ( is_wp_error($user_signon) ){
			$result = json_encode(array('loggedin' => false, 'msg' => esc_html__('Wrong username or password. Please try again!!!', 'findus')));
	    } else {
			wp_set_current_user($user_signon->ID); 
	        $result = json_encode(array('loggedin' => true, 'msg' => esc_html__('Signin successful, redirecting...', 'findus')));
	    }
	...
	public function processForgotPassword() {
		...
		if( empty( $account ) ) {
			$error = esc_html__( 'Enter an username or e-mail address.', 'findus' );
		} else {
			if(is_email( $account )) {
				if( email_exists($account) ) {
					$get_by = 'email';
				} else {
					$error = esc_html__( 'There is no user registered with that email address.', 'findus' );			
				}
			} else if (validate_username( $account )) {
				if( username_exists($account) ) {
					$get_by = 'login';
				} else {
					$error = esc_html__( 'There is no user registered with that username.', 'findus' );				
				}
			} else {
				$error = esc_html__(  'Invalid username or e-mail address.', 'findus' );		
			}
		}	
		...
		if (empty ($error)) {
			...
			if( $update_user ) {
				...
				$to = $user->user_email;
				$subject = esc_html__( 'Your new password', 'findus' );
				$sender = 'From: '.get_option('name').' <'.$from.'>' . "\r\n";
				
				$message = esc_html__( 'Your new password is: ', 'findus' ) .$random_password;
				...
				if( $mail ) {
					$success = esc_html__( 'Check your email address for you new password.', 'findus' );
				} else {
					$error = esc_html__( 'System is unable to send you mail containg your new password.', 'findus' );						
				}
			} else {
				$error =  esc_html__( 'Oops! Something went wrong while updating your account.', 'findus' );
			}
		}
		...
		if ( !is_user_logged_in() ) {
			...
			<div class="account-login">
				<ul class="login-account">
					<li><a href="#apus_login_forgot_tab" class="apus-user-login wel-user"><?php esc_html_e( 'Login','findus' ); ?></a> </li>
					<li class="space">/</li>
                ...
                <div class="setting-account">
                    ...
                    <div class="user-account">
	                    <ul class="user-log">
	                        ...
	                        ?>
	                        <li <?php if(has_nav_menu( 'myaccount-menu' )){ ?> class="last" <?php } ?>><a href="<?php echo esc_url(wp_logout_url(home_url('/'))); ?>"><?php esc_html_e('Log out ','findus'); ?></a></li>
	                    </ul>
	                </div>
                </div>
            </div>
	...
	public function registration_validation( $username, $email, $password, $confirmpassword ) {
		...
		if ( Findus_Recaptcha::is_recaptcha_enabled() && findus_get_config('use_recaptcha_register_form', true) ) {
			$is_recaptcha_valid = array_key_exists( 'g-recaptcha-response', $_POST ) ? Findus_Recaptcha::is_recaptcha_valid( sanitize_text_field( $_POST['g-recaptcha-response'] ) ) : false;
			if ( !$is_recaptcha_valid ) {
				$reg_errors->add('field', esc_html__( 'reCAPTCHA is a required field', 'findus' ) );
			}
		}
		if ( empty( $username ) || empty( $password ) || empty( $email ) || empty( $confirmpassword ) ) {
		    $reg_errors->add('field', esc_html__( 'Required form field is missing', 'findus' ) );
		}

		if ( 4 > strlen( $username ) ) {
		    $reg_errors->add( 'username_length', esc_html__( 'Username too short. At least 4 characters is required', 'findus' ) );
		}

		if ( username_exists( $username ) ) {
	    	$reg_errors->add('user_name', esc_html__( 'That username already exists!', 'findus' ) );
		}

		if ( ! validate_username( $username ) ) {
		    $reg_errors->add( 'username_invalid', esc_html__( 'The username you entered is not valid', 'findus' ) );
		}

		if ( 5 > strlen( $password ) ) {
	        $reg_errors->add( 'password', esc_html__( 'Password length must be greater than 5', 'findus' ) );
	    }

	    if ( $password != $confirmpassword ) {
	        $reg_errors->add( 'password', esc_html__( 'Password must be equal Confirm Password', 'findus' ) );
	    }

	    if ( !is_email( $email ) ) {
		    $reg_errors->add( 'email_invalid', esc_html__( 'Email is not valid', 'findus' ) );
		}

		if ( email_exists( $email ) ) {
		    $reg_errors->add( 'email', esc_html__( 'Email Already in use', 'findus' ) );
		}
	}
	...
	public function processRegister() {
        ...
        if ( 1 > count( $reg_errors->get_error_messages() ) ) {
	        ...
	        if ( ! is_wp_error( $user_id ) ) {
	        	...
	        	if ( findus_get_config('user_register_requires_approval', 'auto') != 'auto') {
	        		$code = findus_random_key();
	                update_user_meta($user_id, 'account_approve_key', $code);
	            	update_user_meta($user_id, 'user_account_status', 'pending');
	            	
	            	if ( findus_get_config('user_register_requires_approval', 'auto') == 'email_approve' ) {
						$user_email = stripslashes( $user_obj->data->user_email );
					} else {
						$user_email = get_option( 'admin_email', false );
					}

					$subject = findus_get_config('user_register_need_approve_subject');
					$subject = str_replace('{user_name}', $user_obj->data->display_name, $subject);

					$content = findus_get_config('user_register_need_approve_content');
					...
					$approve_url = get_permalink(findus_get_config('user_register_approve_page'));
					...
					$header = apply_filters( 'findus-mail-html-header',
						'<!doctype html>
					...
					$footer = apply_filters( 'findus-mail-html-footer',
								'</body>
					</html>' );
	        		...
	        		$subject = findus_get_config('user_register_auto_approve_subject');
					$subject = str_replace('{user_name}', $user_obj->data->display_name, $subject);

					$content = findus_get_config('user_register_auto_approve_content');
					$content = str_replace('{user_name}', $user_obj->data->display_name, $content);
					$content = str_replace('{user_email}', $user_obj->data->user_email, $content);
					$content = str_replace('{website_url}', home_url(), $content);
					$content = str_replace('{website_name}', get_bloginfo( 'name' ), $content);
		        	...
		        	$jsondata = array('loggedin' => true, 'msg' => esc_html__( 'You have registered, redirecting ...', 'findus' ) );
		        ...
		        $jsondata = array('loggedin' => false, 'msg' => esc_html__( 'Register user error!', 'findus' ) );
		    }
	    } else {
	...
	public function process_change_profile_form() {
		check_ajax_referer( 'findus-ajax-edit-profile-nonce', 'security_edit_profile' );
		...
		if ( empty( $nickname ) ) {
			$return['msg'] = '<div class="text-danger">'.esc_html__( 'Nickname is required.', 'findus' ).'</div>';
			echo json_encode($return); exit;
		}

		if ( empty( $email ) ) {
			$return['msg'] = '<div class="text-danger">'.esc_html__( 'E-mail is required.', 'findus' ).'</div>';
			echo json_encode($return); exit;
		}

		do_action('findus_before_change_profile');
			...
			if ( $key !== 'socials' ) {
				...
				if ( $key == 'current_user_avatar' ) {
					if ( is_numeric($value) ) {
						...
						$attachment_id = findus_create_attachment($value);
		...
		$return['msg'] = '<div class="text-success">'.esc_html__( 'Profile has been successfully updated.', 'findus' ).'</div>';
		echo json_encode($return); exit;
	}

	public function process_change_password() {
		check_ajax_referer( 'findus-ajax-change-pass-nonce', 'security_change_pass' );
		...
		if ( empty( $old_password ) || empty( $new_password ) || empty( $retype_password ) ) {
			$return['msg'] = '<div class="text-danger">'.esc_html__( 'All fields are required.', 'findus' ).'</div>';
			echo json_encode($return); exit;
		}

		if ( $new_password != $retype_password ) {
			$return['msg'] = '<div class="text-danger">'.esc_html__( 'New and retyped password are not same.', 'findus' ).'</div>';
			echo json_encode($return); exit;
		}
		
		do_action('findus_before_change_password');

		$user = wp_get_current_user();

		if ( ! wp_check_password( $old_password, $user->data->user_pass, $user->ID ) ) {
			$return['msg'] = '<div class="text-danger">'.esc_html__( 'Your old password is not correct.', 'findus' ).'</div>';
			echo json_encode($return); exit;
		}
		...
		$return['msg'] = '<div class="text-success">'.esc_html__( 'Your password has been successfully changed.', 'findus' ).'</div>';
		echo json_encode($return); exit;
	}
	...
	public function admin_script() {
		wp_enqueue_media();
		wp_enqueue_script( 'findus-upload', get_template_directory_uri() . '/js/upload.js', array( 'jquery' ), '20150330', true );
	}
	/*
	 * back/end settings
	 *
	...
	function user_profile_fields( $user ) {
		...
		?>
		<h3><?php esc_html_e( 'User Profile', 'findus' ); ?></h3>
		...
		<table class="form-table">
			...
			<tr>
				<th>
					<label for="lecturer_job"><?php esc_html_e( 'Avatar', 'findus' ); ?></label>
				</th>
				<td>
					<div class="screenshot-user avatar-screenshot">
			            <?php if ( !empty($avatar_url[0]) ) { ?>
			                <img src="<?php echo esc_url($avatar_url[0]); ?>" alt="<?php esc_attr_e( 'Avatar', 'findus' ); ?>" />
			            <?php } ?>
			        </div>
			        ...
			        <div class="upload_image_action">
			            <input type="button" class="button radius-3x btn btn-theme user-add-image" value="<?php esc_attr_e( 'Add Avatar', 'findus' ); ?>">
			            <input type="button" class="button radius-3x btn btn-theme-second user-remove-image" value="<?php esc_attr_e( 'Remove Avatar', 'findus' ); ?>">
			        </div>
				</td>
			</tr>
			<tr>
				<th>
					<label for="lecturer_mobile"><?php esc_html_e( 'Address', 'findus' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th>
					<label for="lecturer_facebook"><?php esc_html_e( 'Birthday', 'findus' ); ?></label>
				</th>
				<td>
			...
			<?php
				$options = findus_user_social_defaults();
	...
	public static function process_resend_approve_account() {
		...
		if ( empty($user_login) ) {
            echo json_encode(array(
            	'status' => false,
            	'msg' => esc_html__('Username or Email not exactly.', 'findus')
            ));
            die();
        }
        ...
        if ( !empty($user_obj->ID) ) {
	        ...
	        if ( $user_login_auth == 'pending' ) {
				...
				if ( $result ) {
					echo json_encode(array(
		            	'status' => true,
		            	'msg' => esc_html__('Sent a email successfully.', 'findus')
		            ));
		            die();
				} else {
					echo json_encode(array(
		            	'status' => false,
		            	'msg' => esc_html__('Send a email error.', 'findus')
		            ));
		            die();
		        }
	        }
        ...
        echo json_encode(array(
        	'status' => false,
        	'msg' => esc_html__('Your account is not available.', 'findus')
        ));
        die();
	}
	...
	public static function admin_user_auth_callback($user, $password = '') {
		...
		switch ( $status ) {
			case 'pending':
				...
				break;
			case 'denied':
				$denied_message = esc_html__('Your account denied.', 'findus');
	...
	public static function process_approve_user() {
		...
		if ( is_object( $post ) ) {
			if ( strpos( $post->post_content, '[findus_approve_user]' ) !== false ) {
				...
				if ( !$user_id ) {
					$error = array(
						'error' => true,
						'msg' => esc_html__('The user is not exists.', 'findus')
					);
				...
				if ( empty($user) ) {
					...
					if ( $code != $user_code ) {
						$error = array(
							'error' => true,
							'msg' => esc_html__('Code is not exactly.', 'findus')
						);
					}
				}
				...
				if ( empty($error) ) {
					...
					$error = array(
						'error' => false,
						'msg' => esc_html__('Your account approved.', 'findus')
					);
	...
	public static function approve_user( $user_id ) {
		...
		$subject = findus_get_config('user_register_approved_subject');
		$subject = str_replace('{user_name}', $user->data->display_name, $subject);

		$content = findus_get_config('user_register_approved_content');
		...
		do_action( 'findus-new_user_approve_user_approved', $user );
	}
	...
	public static function deny_user( $user_id ) {
		...
		$subject = findus_get_config('user_register_denied_subject');
		$subject = str_replace('{user_name}', $user->data->display_name, $subject);

		$content = findus_get_config('user_register_denied_content');
		$content = str_replace('{user_name}', $user->data->display_name, $content);
		$content = str_replace('{user_email}', $user->data->user_email, $content);
		$content = str_replace('{website_url}', home_url(), $content);
		$content = str_replace('{website_name}', get_bloginfo( 'name' ), $content);
		...
		do_action( 'findus-new_user_approve_user_denied', $user );
	}
	...
	public static function update_user_status( $user, $status ) {
		...
		$do_update = apply_filters( 'findus_new_user_approve_validate_status_update', true, $user_id, $status );
		if ( !$do_update ) {
			return false;
		}

		// where it all happens
		do_action( 'findus_new_user_approve_' . $status . '_user', $user_id );
		do_action( 'findus_new_user_approve_user_status_update', $user_id, $status );
	...
	public static function process_update_user_action() {
		if ( isset( $_GET['action'] ) && in_array( $_GET['action'], array( 'approve', 'deny' ) ) && !isset( $_GET['new_role'] ) ) {
			check_admin_referer( 'findus' );

			$sendback = remove_query_arg( array( 'approved', 'denied', 'deleted', 'ids', 'findus-status-query-submit', 'new_role' ), wp_get_referer() );
			if ( !$sendback ) {
	...
	public static function user_table_actions( $actions, $user ) {
		...
		$approve_link = wp_nonce_url( $approve_link, 'findus' );

		$deny_link = add_query_arg( array( 'action' => 'deny', 'user' => $user->ID ) );
		$deny_link = remove_query_arg( array( 'new_role' ), $deny_link );
		$deny_link = wp_nonce_url( $deny_link, 'findus' );

		$approve_action = '<a href="' . esc_url( $approve_link ) . '">' . esc_html__( 'Approve', 'findus' ) . '</a>';
		$deny_action = '<a href="' . esc_url( $deny_link ) . '">' . esc_html__( 'Deny', 'findus' ) . '</a>';
	...
	public static function add_column( $columns ) {
		$the_columns['user_status'] = esc_html__( 'Status', 'findus' );
	...
	public static function status_column( $val, $column_name, $user_id ) {
		switch ( $column_name ) {
			case 'user_status' :
				...
				if ( $status == 'approved' ) {
					$status_i18n = esc_html__( 'approved', 'findus' );
				} else if ( $status == 'denied' ) {
					$status_i18n = esc_html__( 'denied', 'findus' );
				} else if ( $status == 'pending' ) {
					$status_i18n = esc_html__( 'pending', 'findus' );
				}
				return $status_i18n;
				break;
	...
	public static function status_filter( $which ) {
		$id = 'findus_filter-' . $which;

		$filter_button = submit_button( esc_html__( 'Filter', 'findus' ), 'button', 'findus-status-query-submit', false, array( 'id' => 'findus-status-query-submit' ) );
		$filtered_status = null;
		if ( ! empty( $_REQUEST['findus_filter-top'] ) || ! empty( $_REQUEST['findus_filter-bottom'] ) ) {
			$filtered_status = esc_attr( ( ! empty( $_REQUEST['findus_filter-top'] ) ) ? $_REQUEST['findus_filter-top'] : $_REQUEST['findus_filter-bottom'] );
		}
		$statuses = array('pending', 'approved', 'denied');
		?>
		<label class="screen-reader-text" for="<?php echo esc_attr($id); ?>"><?php _e( 'View all users', 'findus' ); ?></label>
		<select id="<?php echo esc_attr($id); ?>" name="<?php echo esc_attr($id); ?>" style="float: none; margin: 0 0 0 15px;">
			<option value=""><?php _e( 'View all users', 'findus' ); ?></option>
		<?php foreach ( $statuses as $status ) : ?>
			<option value="<?php echo esc_attr( $status ); ?>"<?php selected( $status, $filtered_status ); ?>><?php echo esc_html( $status ); ?></option>
		<?php endforeach; ?>
		</select>
		<?php echo apply_filters( 'findus_filter_button', $filter_button ); ?>
		<style>
			#findus-status-query-submit {
				float: right;
				margin: 2px 0 0 5px;
			}
		</style>
    ...
    public static function filter_by_status( $query ) {
		...
		$filter = null;
		if ( ! empty( $_REQUEST['findus_filter-top'] ) || ! empty( $_REQUEST['findus_filter-bottom'] ) ) {
			$filter = esc_attr( ( ! empty( $_REQUEST['findus_filter-top'] ) ) ? $_REQUEST['findus_filter-top'] : $_REQUEST['findus_filter-bottom'] );
		}
		if ( $filter != null ) {
	...
	public static function register_msg($user) {
		$requires_approval = findus_get_config('user_register_requires_approval', 'auto');

		if ( $requires_approval == 'email_approve' ) {
			return esc_html__('Registration complete. Before you can login, you must active your account sent to your email address.', 'findus');
		} elseif ( $requires_approval == 'admin_approve' ) {
			return esc_html__('Registration complete. Your account has to be confirmed by an administrator before you can login', 'findus');
		} else {
			return esc_html__('Your account has to be confirmed yet.', 'findus');
		}
	}
	...
	public static function login_msg($user) {
		...
		if ( $requires_approval == 'email_approve' ) {
			return sprintf(__('Account account has not confirmed yet, you must active your account with the link sent to your email address. If you did not receive this email, please check your junk/spam folder. <a href="javascript:void(0);" class="findus-resend-approve-account-btn" data-login="%s">Click here</a> to resend the activation email.', 'findus'), $user->user_login );
		} elseif ( $requires_approval == 'admin_approve' ) {
			return esc_html__('Your account has to be confirmed by an administrator before you can login.', 'findus');
		} else {
			return esc_html__('Your account has to be confirmed yet.', 'findus');
		}
	}
