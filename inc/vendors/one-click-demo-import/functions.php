...
function findus_ocdi_import_files() {
    return apply_filters( 'findus_ocdi_files_args', array(
        array(
            'import_file_name'             => 'Findus',
            ...
            'local_import_redux'           => array(
                array(
                    ...
                    'option_name' => 'findus_theme_options',
                ),
            ),
            'import_preview_image_url'     => trailingslashit( get_template_directory_uri() ) . 'screenshot.png',
            'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'findus' ),
            'preview_url'                  => 'http://demoapus-wp.com/findus/place',
        ),
    ) );
}
add_filter( 'pt-ocdi/import_files', 'findus_ocdi_import_files' );

function findus_ocdi_after_import_setup( $selected_import ) {
    ...
    if ( file_exists($file) ) {
        findus_ocdi_import_settings($file);
    }
...
add_action( 'pt-ocdi/after_import', 'findus_ocdi_after_import_setup' );

function findus_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        findus_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        findus_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function findus_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function findus_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function findus_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'findus_ocdi_before_widgets_import' );