...
function findus_listing_review_enable($post_id = null) {
    if ( empty($post_id) ) {
        $post_id = get_the_ID();
    }
    if ( ! comments_open($post_id) || !findus_get_config('listing_review_enable', true) ) {
        return false;
    }
    return true;
}

function findus_listing_review_rating_enable() {
    $return = false;
    if ( findus_get_config('listing_review_enable', true) && findus_get_config('listing_review_enable_rating', true) ) {
        $return = true;
    }
    return apply_filters('findus_listing_review_rating_enable', $return);
}

function findus_listing_review_rating_field($default_val = array()) {
    global $post;
    ...
    $categories = findus_listing_review_categories();
    if ( findus_listing_review_rating_enable() && $categories ) {
        ?>
...
function findus_listing_review_rating_field_display() {
    global $post;
    if ( $post->post_type == 'job_listing' ) {
        echo trim(findus_listing_review_rating_field());
    }
}



function findus_listing_review_categories() {
    $category_titles = findus_get_config('listing_review_category_title');
    $category_keys = findus_get_config('listing_review_category_key');
    $return = array();
...
// comment template
function findus_listing_comments_template_loader($template) {
    if ( get_post_type() !== 'job_listing' ) {
        return $template;
    }
    return get_template_directory() . '/job_manager/single/parts/reviews.php';
}
add_filter( 'comments_template', 'findus_listing_comments_template_loader' );

// comment list
function findus_listing_comments( $comment, $args, $depth ) {
...
function findus_my_review( $comment, $args, $depth ) {
...
// add comment meta
function findus_add_custom_comment_field( $comment_id, $comment_approved, $commentdata ) {
    ...
    if ( $post->post_type == 'job_listing' && isset($_POST['rating']) ) {
        ...
        $average_rating = findus_get_total_rating( $post_id );
        update_post_meta( $post_id, '_average_rating', $average_rating );

        $avg_ratings = findus_get_total_ratings($post_id);
        update_post_meta( $post_id, '_average_ratings', $avg_ratings );
        
    }
}
add_action( 'comment_post', 'findus_add_custom_comment_field', 10, 3 );


function findus_add_custom_comment_average_rating($comment) {
...
add_action( 'comment_unapproved_to_approved', 'findus_add_custom_comment_average_rating', 10 );
add_action( 'comment_approved_to_unapproved', 'findus_add_custom_comment_average_rating', 10 );
add_action( 'comment_approved_to_trash', 'findus_add_custom_comment_average_rating', 10 );
add_action( 'comment_trash_to_approved', 'findus_add_custom_comment_average_rating', 10 );
add_action( 'comment_approved_to_spam', 'findus_add_custom_comment_average_rating', 10 );
add_action( 'comment_spam_to_approved', 'findus_add_custom_comment_average_rating', 10 );

function findus_get_review_comments( $args = array() ) {
...
function findus_get_total_reviews( $post_id ) {
...
function findus_get_total_rating( $post_id ) {
    $args = array( 'post_id' => $post_id );
    $comments = findus_get_review_comments($args);
    if (empty($comments)) {
        return 0;
...
function findus_get_total_ratings( $post_id ) {
    $args = array( 'post_id' => $post_id );
    $comments = findus_get_review_comments($args);
    if (empty($comments)) {
        return;
...
function findus_get_total_rating_by_user( $user_id ) {
...
function findus_get_detail_ratings( $post_id ) {
    global $wpdb;
...
function findus_print_review( $rate, $type = '', $nb = 0 ) {
    $rating_mode = findus_get_config('listing_review_mode', 10);
    ...
    <div class="review-stars-rated-wrapper">
        ...
        <?php if ($type == 'list') { ?>
            <span class="nb-review"><?php echo sprintf(_n('(%d Review)', '(%d Reviews)', $nb, 'findus'), $nb); ?></span>
        <?php } ?>
    </div>
    <?php
}
...
// comment like
function findus_check_comment_like_user($comment_id) {
    $meta_key = '_apus_like';
    $return = false;
...
function findus_comment_like_user($comment_id) {
    $meta_key = '_apus_like';
    $return = true;
    if ( is_user_logged_in() ) {
        $user_id = get_current_user_id();
...
function findus_comment_like() {
    ...
    $count = 0;
    if ( !empty($comment_id) ) {
        $user_like = findus_comment_like_user($comment_id);
        $tract = -1;
        if ( $user_like ) {
            $tract = 1;
        }
    ...
    if ( $tract == -1) {
        $return = array(
            ...
            'dtitle' => sprintf(esc_html__('Like %d', 'findus'), $count),
        );
    } else {
        $return = array(
...
add_action( 'wp_ajax_findus_comment_like', 'findus_comment_like' );
add_action( 'wp_ajax_nopriv_findus_comment_like', 'findus_comment_like' );


// comment dislike
function findus_check_comment_dislike_user($comment_id) {
...
function findus_comment_dislike_user($comment_id) {
    $meta_key = '_apus_dislike';
    $return = true;
    if ( is_user_logged_in() ) {
        $user_id = get_current_user_id();
...
function findus_comment_dislike() {
    check_ajax_referer( 'findus-ajax-nonce', 'security' );
    $comment_id = !empty($_POST['comment_id']) ? $_POST['comment_id'] : 0;
    $count_key = '_apus_dislike';
    $count = 0;
    if ( !empty($comment_id) ) {
        $user_dislike = findus_comment_dislike_user($comment_id);
        $tract = -1;
        if ( $user_dislike ) {
            $tract = 1;
        }
    ...
    if ( $tract == -1) {
        $return = array(
            ...
            'dtitle' => sprintf(esc_html__('Dislike %d', 'findus'), $count),
        );
    } else {
        $return = array(
...
add_action( 'wp_ajax_findus_comment_dislike', 'findus_comment_dislike' );
add_action( 'wp_ajax_nopriv_findus_comment_dislike', 'findus_comment_dislike' );


// comment love
function findus_check_comment_love_user($comment_id) {
    $meta_key = '_apus_love';
    $return = false;
    if ( is_user_logged_in() ) {
        $user_id = get_current_user_id();
...
function findus_comment_love_user($comment_id) {
    $meta_key = '_apus_love';
    $return = true;
    if ( is_user_logged_in() ) {
        $user_id = get_current_user_id();
...
function findus_comment_love() {
    check_ajax_referer( 'findus-ajax-nonce', 'security' );
    $comment_id = !empty($_POST['comment_id']) ? $_POST['comment_id'] : 0;
    $count_key = '_apus_love';
    $count = 0;
    if ( !empty($comment_id) ) {
        $user_love = findus_comment_love_user($comment_id);
        $tract = -1;
        if ( $user_love ) {
            $tract = 1;
        }
    ...
    if ( $tract == -1) {
        $return = array(
            ...
            'dtitle' => sprintf(esc_html__('Love %d', 'findus'), $count),
        );
    } else {
        $return = array(
...
add_action( 'wp_ajax_findus_comment_love', 'findus_comment_love' );
add_action( 'wp_ajax_nopriv_findus_comment_love', 'findus_comment_love' );


function findus_paginate_links( $args = array() ) {
    global $wp_rewrite;
...
function findus_check_is_review_owner($comment) {
...
add_action( 'wp_ajax_findus_comment_edit', 'findus_comment_edit' );
add_action( 'wp_ajax_nopriv_findus_comment_edit', 'findus_comment_edit' );
function findus_comment_edit() {
    check_ajax_referer( 'findus-ajax-nonce', 'security' );
...
add_action( 'wp_ajax_findus_process_comment_edit', 'findus_process_comment_edit' );
add_action( 'wp_ajax_nopriv_findus_process_comment_edit', 'findus_process_comment_edit' );
function findus_process_comment_edit() {

    check_ajax_referer( 'edit-review', 'edit-review-nonce' );

    if ( !is_user_logged_in() ) {
        $return = array( 'status' => false, 'msg' => esc_html__('Please login to edit this review.', 'findus') );
        echo wp_json_encode($return);
        exit;
    }
    if ( empty($_POST['comment']) ) {
        $return = array( 'status' => false, 'msg' => esc_html__('Please enter review comment.', 'findus') );
        echo wp_json_encode($return);
        exit;
    }
    ...
    if ( !$comment_id || !$post_id ) {
        $return = array( 'status' => false, 'msg' => esc_html__('Review or Post is not exists.', 'findus') );
        echo wp_json_encode($return);
        exit;
    }
    ...
    if ( empty($comment['comment_post_ID']) || $comment['comment_post_ID'] != $post_id ) {
        $return = array( 'status' => false, 'msg' => esc_html__('Review is not exists.', 'findus') );
        echo wp_json_encode($return);
        exit;
    }
    ...
    $result = wp_update_comment($comment_args);
        ...
        findus_add_custom_comment_field( $comment_id, true, $comment );
        ...
        if ( $rating > 0 ) {
            $rating_mode = findus_get_config('listing_review_mode', 10);
            ob_start();
            findus_display_listing_review_html($rating, $rating_mode);
            $review_html = ob_get_clean();
            $comment_data['rating_ouput'] = '<div class="star-rating " title="'.sprintf( esc_attr__( 'Rated %d out of %d', 'findus' ), $rating, $rating_mode ).'">'.$review_html.'</div>';
        }

        $return = array( 'status' => true, 'msg' => esc_html__('Update review successful.', 'findus'), 'comment_data' => $comment_data );
        echo wp_json_encode($return);
        exit;
