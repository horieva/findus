...
class Findus_Bookmark {
	
	public static function init() {
		add_action( 'wp_ajax_findus_add_bookmark', array(__CLASS__, 'add_bookmark') );
		add_action( 'wp_ajax_nopriv_findus_add_bookmark', array(__CLASS__, 'add_bookmark') );
		add_action( 'wp_ajax_findus_remove_bookmark', array(__CLASS__, 'remove_bookmark') );
		add_action( 'wp_ajax_nopriv_findus_remove_bookmark', array(__CLASS__, 'remove_bookmark') );
	...
	public static function add_bookmark() {
		...
		do_action('findus_before_add_bookmak');
		if ( isset($_GET['post_id']) && $_GET['post_id'] ) {
			self::save_wishlist($_GET['post_id']);
			$result['status'] = 'success';
			$result['msg'] = esc_html__('Added bookmark successful.', 'findus');
		} else {
			$result['status'] = 'error';
			$result['msg'] = esc_html__('Add bookmark error.', 'findus');
		}
	...
	public static function remove_bookmark() {
		check_ajax_referer( 'findus-ajax-nonce', 'security' );
		do_action('findus_before_remove_bookmak');
		if ( isset($_GET['post_id']) && $_GET['post_id'] ) {
			...
			$result['status'] = 'success';
			$result['msg'] = esc_html__('Removed bookmark successful.', 'findus');
		} else {
			$result['status'] = 'error';
			$result['msg'] = esc_html__('Remove bookmark error.', 'findus');
		}
		echo json_encode($result);
		die();
	}
	...
	public static function settings($args) {
		$args['job_pages'][1][] = array(
			...
			'std' 		=> '',
			'label' 	=> esc_html__( 'Apus Bookmark Listings Page', 'findus' ),
			'desc'		=> esc_html__( 'Select the page where you have placed the visual composer element "Apus My Bookmark". This lets the plugin know where the job listings page is located.', 'findus' ),
			'type'      => 'page'
		);
		return $args;
	}
	...
	public static function display_bookmark_btn( $post ) {
		...
		if ( !is_user_logged_in() ) {
			...
			$added = Findus_Bookmark::check_listing_added($post_id);
			if ($added) {
		...
		<div class="listing-btn-wrapper listing-bookmark">
			<a href="#apus-bookmark-add" class="<?php echo esc_attr($class); ?>" data-id="<?php echo esc_attr($post_id); ?>">
				<i class="<?php echo esc_attr($icon_class); ?>"></i><span class="bookmark-text"><?php esc_html_e( 'Save', 'findus' ); ?></span>
			</a>
		</div>
		<?php
	}
	...
	public static function display_bookmark_btn_detail() {
		...
		<a href="#apus-bookmark-add" class="<?php echo esc_attr($class); ?> btn-bookmark btn" data-id="<?php echo esc_attr($post_id); ?>">
			...
			<span class="bookmark-text"><?php esc_html_e( 'Save', 'findus' ); ?></span>
		</a>
		<?php
	}
}

Findus_Bookmark::init();