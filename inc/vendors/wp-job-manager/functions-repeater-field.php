...
function findus_repeater_rows_html( $group_name, $fields, $data ) {
	?>
	<div class="wc-job-manager-repeater-rows">
			...
			<?php
				if ( $data ) {
					foreach ( $data as $item ) {
						...
						echo '<a href="" class="delete-repeat-row">'.esc_html__('Delete this row', 'findus').'</a>';
						echo '</div>';
					}
				}
			?>
		...
		<div class="submit">
			<input type="submit" class="button repeate_field_add_row" value="<?php printf(esc_attr__( 'Add %s', 'findus' ), $group_name ); ?>" data-row="<?php
				ob_start();
				echo '<div class="repeater-field row">';
 ...
 * @since 1.11.3
 */
function findus_save_repeater_row( $post_id, $meta_key, $fields ) {
	$items            = array();
...
// front end
function findus_get_repeater_field( $field_prefix, $field ) {
	$fields = $field['fields'];
			...
			foreach ( $fields as $key => $field ) {
				...
				switch ( $field['type'] ) {
					...
					case 'file' :
						...
						$file = findus_upload_file( $field_name, $field );
...
function findus_upload_file( $field_key, $field ) {
...
function findus_get_posted_repeater_field() {
	return 'findus_get_repeater_field';
}
add_filter( 'job_manager_get_posted_repeater_field', 'findus_get_posted_repeater_field' );