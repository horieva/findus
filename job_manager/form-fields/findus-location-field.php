...
wp_localize_script( 'listing-location', 'listing_opts', array(
	...
	'latitude' => findus_get_config('listing_map_latitude', ''),
	'longitude' => findus_get_config('listing_map_longitude', ''),
	)
);
...
$mapbox_style = '';
$custom_style = '';
if ( findus_get_config('listing_map_style_type', '') == 'mapbox' ) {
	$mapbox_token = findus_get_config('listing_mapbox_token', '');
	$mapbox_style = findus_get_config('listing_mapbox_style', '');
} else {
	$custom_style = findus_get_config('listing_map_custom_style', '');
}
wp_localize_script( 'listing-location', 'findus_listing_map_opts', array(
...
<div class="findus-location-field">
	<div class="row flex-middle-sm">
		<div class="col-md-6 col-xs-12">
			<fieldset>
				<label><?php esc_html_e( 'Friendly Location', 'findus' ); ?></label>
				<div class="field">
			...
			<div class="findus-location-field-inner">
				...
				<span class="find-me-location" title="<?php echo esc_attr__('Find Me', 'findus'); ?>"><?php get_template_part( 'images/icon/location' ); ?></span>
				<span class="loading-me"></span>
				
			</div>
			<fieldset>
				<label><?php esc_html_e( 'Latitude', 'findus' ); ?></label>
				<div class="field">
					<input class="geo_latitude" placeholder="<?php esc_attr_e('51.4980073', 'findus'); ?>"  name="geo_latitude" value="<?php echo esc_attr( $geo_latitude); ?>" type="text">
				</div>
			</fieldset>
			<fieldset>
				<label><?php esc_html_e( 'Longitude', 'findus' ); ?></label>
				<div class="field">
					<input class="geo_longitude" placeholder="<?php esc_attr_e('51.4980073', 'findus'); ?>" name="geo_longitude" value="<?php echo esc_attr( $geo_longitude ); ?>" type="text">
				</div>
			</fieldset>
		</div>
		<div class="col-md-6 col-xs-12">
