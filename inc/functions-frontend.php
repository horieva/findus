...
if ( ! function_exists( 'findus_post_tags' ) ) {
	function findus_post_tags() {
		$posttags = get_the_tags();
		if ( $posttags ) {
			echo '<span class="entry-tags-list"><strong>'.esc_html__( 'Tags: ' , 'findus' ).'</strong> ';
			$size = count( $posttags );
			$count = 1;
...
if ( ! function_exists( 'findus_breadcrumb_title' ) ) {
	function findus_breadcrumb_title() {
		$title = '';
		...
		if ( !is_front_page() || is_paged()) {
			...
			elseif (is_category()) {
				...
				$title = esc_html__('Search results for ','findus') . '"'.get_search_query().'"';
			} elseif ( is_tag() ) {
				$title = esc_html__('Posts tagged', 'findus') . '"' . single_tag_title('', false) . '"';
			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata($author);
				$title = esc_html__('Profile', 'findus');
			} elseif ( is_404() ) {
				$title = esc_html__('Error 404', 'findus');
			}
...
if ( ! function_exists( 'findus_breadcrumbs' ) ) {
	function findus_breadcrumbs($show_title = true) {
		$delimiter = ' ';
		$home = esc_html__('Home', 'findus');
		...
		if ( !is_front_page() || is_paged()) {
			...
			if (is_category()) {
				...
				if (is_tax('job_listing_category') || is_tax('job_listing_tag') || is_tax('job_listing_amenity') || is_tax('job_listing_region') || is_tax('job_listing_type')) {
					...
					if ($thisCat->parent != 0) {
						echo '<li><a href="' . findus_get_listings_page_url() . '">' . esc_html__('Listings', 'findus') . '</a></li> ' . $delimiter . ' ';
						...
						echo '<li><a href="' . findus_get_listings_page_url() . '">' . esc_html__('Listings', 'findus') . '</a></li>';
						echo trim($before.single_cat_title('', false) . $after);
					}
				...
				echo trim($before) . esc_html__('Search', 'findus') . $after;
			} elseif ( is_tag() ) {
				echo trim($before) . esc_html__('Posts tagged "', 'findus'). single_tag_title('', false) . '"' . $after;
			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata($author);
				echo trim($before) . $userdata->display_name . $after;
			} elseif ( is_404() ) {
				echo trim($before) . esc_html__('Error 404', 'findus') . $after;
			} elseif ( is_home() ) {
				echo trim($before) . esc_html__('blog', 'findus') . $after;
			}
			echo '</ol>';
		}
	}
...
function findus_listing_breadcrumbs() {
	$delimiter = ' ';
	$home = esc_html__('Home', 'findus');
	global $post;
	$homeLink = esc_url( home_url('/') );
	...
	echo '<ol class="breadcrumb">';
		echo '<li><a href="' . $homeLink . '">' . $home . '</a> ' . $delimiter . '</li> ';
		echo '<li><a href="'.findus_get_listings_page_url().'">'.esc_html__( 'Listings', 'findus' ).'</a> ' . $delimiter . '</li> ';
...
if ( ! function_exists( 'findus_render_breadcrumbs' ) ) {
	function findus_render_breadcrumbs($type = 'normal') {
		global $post, $findus_page_title;
		$show = true;
		$style = array();
		$has_img = '';
		$des = '';
		...
		if ( is_page() && is_object($post) ) {
			...
			$breadcrumb_img = findus_get_config('blog_breadcrumb_image');
	        $breadcrumb_color = findus_get_config('blog_breadcrumb_color');
	        $style = array();
	        if ( $breadcrumb_color ) {
			...
			$show = findus_get_config('show_listing_breadcrumbs', true);
			$listing_archive = 1;
			...
			$breadcrumb_img = findus_get_config('listing_breadcrumb_image');
	        $breadcrumb_color = findus_get_config('listing_breadcrumb_color');
	        $style = array();
	        if ( $breadcrumb_color ) {
	            $style[] = 'background-color:'.$breadcrumb_color;
	        }
			...
			$show = findus_get_config('show_profile_breadcrumbs', true);
			if ( !$show  ) {
				return ''; 
			}
			$breadcrumb_img = findus_get_config('profile_breadcrumb_image');
	        $breadcrumb_color = findus_get_config('profile_breadcrumb_color');
	        $style = array();
	        if ( $breadcrumb_color ) {
	            $style[] = 'background-color:'.$breadcrumb_color;
	        }
			...
			$show = findus_get_config('show_404_breadcrumbs', true);
			if ( !$show  ) {
				return ''; 
			}
			$breadcrumb_img = findus_get_config('404_breadcrumb_image');
	        $breadcrumb_color = findus_get_config('404_breadcrumb_color');
	        $style = array();
	        if ( $breadcrumb_color ) {
	            $style[] = 'background-color:'.$breadcrumb_color;
	        }
		...
		echo '<section id="apus-breadscrumb" class="apus-breadscrumb '.$has_img.'"'.$estyle.'><div class="apus-inner-bread"><div class="wrapper-breads"><div class="'.$container_class.'"><div class="breadscrumb-inner text-center">';
			if ($type == 'listing' || (isset($listing_single) && $listing_single == 1) ) {
				findus_listing_breadcrumbs();
			} else {
				$findus_page_title = $title = findus_breadcrumb_title();
				if(!empty($title)){
...
if ( ! function_exists( 'findus_paging_nav' ) ) {
	function findus_paging_nav() {
		...
		$links = paginate_links( array(
			...
			'prev_text' => '<i class="fas fa-long-arrow-alt-left"></i>'.esc_html__( 'Prev', 'findus' ),
			'next_text' => esc_html__( 'Next', 'findus' ).'<i class="fas fa-long-arrow-alt-right"></i>',
		) );
		...
		<nav class="navigation paging-navigation" role="navigation">
			<h1 class="screen-reader-text hidden"><?php esc_html_e( 'Posts navigation', 'findus' ); ?></h1>
...
if ( !function_exists('findus_comment_form') ) {
	function findus_comment_form($arg, $class = 'btn-theme') {
		global $post;
...
if (!function_exists('findus_list_comment') ) {
	function findus_list_comment($comment, $args, $depth) {
...
function findus_comment_field_to_bottom( $fields ) {
	$comment_field = $fields['comment'];
	unset( $fields['comment'] );
	$fields['comment'] = $comment_field;
	return $fields;
}
add_filter( 'comment_form_fields', 'findus_comment_field_to_bottom' );

function findus_get_blogs_layout_type() {
	$layout = findus_get_config( 'blog_display_mode', 'list' );
	$layout = !empty($layout) ? $layout : 'list';
	return $layout;
}

function findus_get_blog_thumbsize() {
	$thumbsize = findus_get_config( 'blog_item_thumbsize', '' );
	return $thumbsize;
}
 ...
 */
function findus_create_placeholder($size) {
...
function findus_display_sidebar_left( $sidebar_configs ) {
	if ( isset($sidebar_configs['left']) ) : ?>
		<div class="<?php echo esc_attr($sidebar_configs['left']['class']) ;?>">
		  	<aside class="sidebar sidebar-left" itemscope="itemscope" itemtype="http://schema.org/WPSideBar">
		  		<div class="close-sidebar-btn hidden-lg hidden-md"> <i class="fas fa-times"></i> <span><?php esc_html_e('Close', 'findus'); ?></span></div>
...
function findus_display_sidebar_right( $sidebar_configs ) {
...
function findus_before_content( $sidebar_configs ) {
	if ( isset($sidebar_configs['left']) || isset($sidebar_configs['right']) ) : ?>
		<a href="javascript:void(0)" class="mobile-sidebar-btn hidden-lg hidden-md"> <i class="fas fa-bars"></i> <?php echo esc_html__('Show Sidebar', 'findus'); ?></a>
