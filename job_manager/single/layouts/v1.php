<article id="post-<?php the_ID(); ?>" <?php post_class('detail-listing-place detail-listing-place-v1 apus-single-listing-wrapper'); ?> <?php echo trim(findus_display_map_data($post)); ?>>
	<?php
	global $post;
	$attach_id = get_post_thumbnail_id( $post->ID );
	$style = '';
	if ( !empty($attach_id) ) {
		$img = wp_get_attachment_image_src($attach_id, 'findus-image-full');
		if ( !empty($img[0]) ) {
	...
	<?php
		...
		$contents = findus_get_content_sort();
		if ( !empty($contents) ) {
			foreach ($contents as $key => $title) {
				$content = trim(findus_listing_display_part($key));
				if ( !empty($content) ) {
	...
	<div class="container detail-content">
		<div class="entry-listing-content">
			...
			wp_link_pages( array(
				'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'findus' ),
				'after'  => '</div>',
			) ); ?>
		</div>
	</div><!-- .entry-content -->
