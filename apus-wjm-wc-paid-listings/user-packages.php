...
if ( $user_packages ) : ?>
	<div class="widget widget-your-packages">
		<h2 class="widget-title hidden"><?php esc_html_e( 'Your Packages', 'findus' ); ?></h2>
		<ul class="job_packages">
			...
			?>
						...
						<li class="user-job-package">
							...
							<?php
								if ( $listings_limit ) {
									printf( _n( '%s listing posted out of %d', '%s listings posted out of %d', $package_count, 'findus' ), $package_count, $listings_limit );
								} else {
									printf( _n( '%s listing posted', '%s listings posted', $package_count, 'findus' ), $package_count );
								}

								if ( $listings_duration ) {
									printf(  ', ' . _n( 'listed for %s day', 'listed for %s days', $listings_duration, 'findus' ), $listings_duration );
								}
