...
$days = findus_get_day_hours($hours);
...
?>
	<?php foreach ($days as $day => $times) {
			...
			$day_time_heading = '';
			if ( $times == 'open' ) {
				$day_time = '<span class="open-text">'.esc_html__('Open All Day', 'findus').'</span>';
			} elseif ( $times == 'closed' ) {
				$day_time = '<span class="close-text">'.esc_html__('Closed All Day', 'findus').'</span>';
	...
	?>
			<div id="listing-hours" class="listing-hours widget">
				<h2 class="widget-title">
						<span><?php esc_html_e('Today', 'findus'); ?></span>
						<span class="pull-right hour-present flex">
							<?php findus_display_time_status($post); ?>
				...
				<div class="listing-hours-inner1 flex-top-lg box-inner clearfix">
					...
					<?php do_action('findus-single-listing-hours', $post); ?>
				</div>
			</div>
		<?php } ?>
<?php endif; ?>