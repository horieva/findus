...
class Findus_Submittion{
	...
	public static function init() {
		...
		if ( function_exists('apusfindus_addmetaboxes') ) {
			apusfindus_addmetaboxes( array( __CLASS__, 'metaboxes' ) );
		}

		add_filter('apusfindus-types-add_custom_fields', array( __CLASS__, 'admin_fields' ), 10 );
		// admin fields
		add_action( 'job_manager_input_findus-location', array( __CLASS__, 'input_location_fields' ), 10, 2 );
		...
		add_action( 'job_manager_input_findus-regions', array( __CLASS__, 'input_regions_fields' ), 10, 2 );
		...
		// region field
		add_action( 'wp_ajax_findus_process_change_region', array( __CLASS__, 'process_change_region' ) );
		add_action( 'wp_ajax_nopriv_findus_process_change_region', array( __CLASS__, 'process_change_region' ) );
	...
	public static function change_post_type_label($args) {
		$singular = esc_html__( 'Listing', 'findus' );
		$plural   = esc_html__( 'Listings', 'findus' );

		$args['labels']      = array(
			'name'               => $plural,
			'singular_name'      => $singular,
			'menu_name'          => $plural,
			'all_items'          => sprintf( esc_html__( 'All %s', 'findus' ), $plural ),
			'add_new'            => esc_html__( 'Add New', 'findus' ),
			'add_new_item'       => sprintf( esc_html__( 'Add %s', 'findus' ), $singular ),
			'edit'               => esc_html__( 'Edit', 'findus' ),
			'edit_item'          => sprintf( esc_html__( 'Edit %s', 'findus' ), $singular ),
			'new_item'           => sprintf( esc_html__( 'New %s', 'findus' ), $singular ),
			'view'               => sprintf( esc_html__( 'View %s', 'findus' ), $singular ),
			'view_item'          => sprintf( esc_html__( 'View %s', 'findus' ), $singular ),
			'search_items'       => sprintf( esc_html__( 'Search %s', 'findus' ), $plural ),
			'not_found'          => sprintf( esc_html__( 'No %s found', 'findus' ), $plural ),
			'not_found_in_trash' => sprintf( esc_html__( 'No %s found in trash', 'findus' ), $plural ),
			'parent'             => sprintf( esc_html__( 'Parent %s', 'findus' ), $singular )
		);
		$args['description'] = sprintf( esc_html__( 'This is where you can create and manage %s.', 'findus' ), $plural );
	...
	public static function add_custom_fields($fields) {
		if ( findus_get_config('claim_enable') ) {
			$fields['_claimed'] = array(
				'label'       => esc_html__( 'Claimed:', 'findus' ),
				'type'        => 'checkbox',
				'description' => esc_html__( 'The owner has been verified.', 'findus' ),
				'priority'    => 2.3,
				'required' => false,
			);
		}
	...
	public static function metaboxes( $post_type ) {
        ...
        if ( in_array( $post_type, $post_types ) && function_exists('apusfindus_addmetabox') ) {

            apusfindus_addmetabox(
            	'job_listing_images',
                esc_html__( 'Listing Images', 'findus' ),
            ...
            if ( class_exists('ApusFindus_Custom_Fields') ) {
	            $fields = ApusFindus_Custom_Fields::add_custom_fields(array());
	            if ( !empty($fields) ) {
	            	foreach ($fields as $key => $field) {
	            		if ( $field['type'] == 'repeater' ) {
	            			$label = !empty($field['label']) ? $field['label'] : esc_html__('Repeater', 'findus');
	            			apusfindus_addmetabox(
	            				'job_listing_'.$key,
				                $label,
	            		...
	            		} elseif ( $field['type'] == 'findus-menu-prices' ) {
	            			$label = !empty($field['label']) ? $field['label'] : esc_html__('Menu Prices', 'findus');
	            			apusfindus_addmetabox(
	            				'job_listing_'.$key,
				                $label,
	            		...
	            		} elseif ( $field['type'] == 'findus-hours' ) {
	            			$label = !empty($field['label']) ? $field['label'] : esc_html__('Hours Operation', 'findus');
	            			apusfindus_addmetabox(
	            				'job_listing_'.$key,
				                $label,
	...
	public static function render_meta_box_repeater( $post, $args ) {
		...
		findus_repeater_rows_html( $field['label'], $field['fields'], get_post_meta( $post->ID, $key, true ) );
	}
	...
	public static function render_meta_box_settings( $post ) {
		...
		if ( !empty($post) ) {
			...
			$fields = get_option('findus_custom_fields_data', true);
			$has_job_logo = self::check_type_has_field($fields, 'job_logo');
			if ( $has_job_logo ) {
				$job_logo = get_post_meta($post->ID, '_job_logo', true);

    	?>
		    	<div class="findus-listing-side-meta">
		    		<h3><?php esc_html_e('Listing Logo', 'findus'); ?></h3>
		    		...
		    		<button id="upload-image-logo" type="button" class="button"><?php esc_html_e('Add logo image', 'findus'); ?></button>
    	...
    	?>		
		    	<div class="findus-listing-side-meta">
		    		<h3><?php esc_html_e('Gallery Images', 'findus'); ?></h3>
		    		...
		    		<button id="upload-image-gallery" type="button" class="button"><?php esc_html_e('Add gallery images', 'findus'); ?></button>
		    	</div>
	    	<?php }
	    	?>
	    <?php } ?>
	...
	public static function input_hours_fields($key, $field) {
		...
		<div class="form-field full-form-field">
			...
			get_job_manager_template( 'form-fields/findus-hours-field.php', array('key' => $key, 'field' => $field) );
			?>
		</div>
	...
	public static function input_location_fields($key, $field) {
		...
		<div class="form-field location-form-field full-form-field">
			...
			get_job_manager_template( 'form-fields/findus-location-field.php', array('key' => $key, 'field' => $field) );
			?>
		</div>
	...
	public static function input_menu_prices_fields($key, $field) {
		...
		<div class="form-field full-form-field">
			...
			get_job_manager_template( 'form-fields/findus-menu-prices-field.php', array('key' => $key, 'field' => $field) );
			?>
		</div>
	...
	public static function input_regions_fields($key, $field) {
		...
		<div class="form-field">
			...
			get_job_manager_template( 'form-fields/findus-regions-field.php', array('key' => $key, 'field' => $field) );
			?>
		</div>
	...
	public static function process_change_region() {
		...
		$region_text = findus_get_config('submit_listing_region_'.$next.'_field_label');
		$placeholder = !empty($_POST['placeholder']) ? $_POST['placeholder'] : sprintf(esc_html__('Please select %s', 'findus'), $region_text);
	...
	public static function listing_submit( $id, $values ) {
		if ( isset($values[ 'job' ][ 'job_cover_image' ]) && !empty($values[ 'job' ][ 'job_cover_image' ]) ) {
			...
			if ( is_numeric($job_cover_image) ) {
				...
				$attachment_id = findus_get_attachment_id_from_url( $job_cover_image );
			}
		...
		if ( isset($values[ 'job' ][ 'job_logo' ]) && !empty($values[ 'job' ][ 'job_logo' ]) ) {
			...
			if ( is_numeric($job_logo) ) {
				...
				$attachment_id = findus_get_attachment_id_from_url( $job_logo );
			}
		...
		if ( isset($values[ 'job' ][ 'job_gallery_images' ]) && !empty($values[ 'job' ][ 'job_gallery_images' ]) ) {
			...
			if ( is_numeric( $job_gallery_images ) ) {
				$attach_id = findus_get_attachment_id_from_url( $job_gallery_images );
				...
				foreach ( $job_gallery_images as $key => $url ) {
					if ( is_numeric( $url ) ) {
						...
						$attach_id = findus_get_attachment_id_from_url( $url );
						if ( ! empty( $attach_id ) && is_numeric( $attach_id ) ) {
							$gallery_images[] = $attach_id;
						}
					}
	...
	public static function listing_update( $id, $values ) {
		...
		if ( class_exists('ApusFindus_Custom_Fields') ) {
	        ...
	        if ( !empty($fields) ) {
	        	foreach ($fields as $key => $field) {
	        		if ( $field['type'] == 'repeater' ) {
	        			findus_save_repeater_row( $id, $key, $field['fields'] );
	        		}
	        	}
	        }
        }
	...
	public static function job_manager_dropdown_categories( $args = '' ) {
		$defaults = array(
			...
			'placeholder'     => esc_html__( 'Choose a category&hellip;', 'findus' ),
			'no_results_text' => esc_html__( 'No results match', 'findus' ),
			'multiple_text'   => esc_html__( 'Select Some Options', 'findus' ),
			'meta_query'   => ''
		);
